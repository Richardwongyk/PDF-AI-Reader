...
- 2026-05-24 公式工具环境安装尝试失败后，已删除所有临时 PDF 工具环境：
  `pdf_formula_tools_310`、`pdf_tool_mineru310`、`pdf_tool_magic310`、
  `pdf_tool_paddle310`、`pdf_tool_pek310`。
- 最后一次确认时，`conda env list` 中没有任何 `pdf_tool_*` 或 `pdf_formula_*` 环境。
- 如果新会话要重新装外部工具，必须先 `conda env list` 和检查残留进程，不能假设旧环境可用。
...
- `r5_knowledge_incremental_update`：设计层轮次，代码尚未显式落地；accepted 高置信结果变化后应增量更新全文索引。
...
3. 执行 `conda env list`，确认没有半残留工具环境。
