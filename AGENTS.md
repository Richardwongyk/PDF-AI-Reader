...
`docs/next_session_handoff.md`、`docs/current_goal_and_next_steps.md`、
`docs/tiny_born_digital_math_model_engineering.md`、
`docs/tinybdmath_ai_math_latex_structure_scope.md`、`TODO.md`、
...
- 2026-05-28 性能修复尝试暴露 P0 退化：Napkin 前台 400x 测试在极大缩放/快速滚动/跳页时
  出现大页面 tile-only 首帧空瓦片，用户看到黑底/空白页。后续如果回到 UI 性能，必须先修
  “滚动时中间页始终可见”，不能继续以牺牲阅读体验换性能指标。
...
- 2026-06-02 已收缩 TinyBDMath 当前入口。当前 TinyBDMath 代码入口只保留插桩/graph rows、
  CSLT target tree、PDF graph 到 CSLT alignment、Graph Parser、candidate-only r2a、
  LaTeX candidate decode 和公式级评估。目录保留/可删边界见 `docs/workspace_inventory.md`。
...
- 2026-06-05 已提交并推送 `a5091f8 Add AI dock panel controls`：右侧 `AI 工具集`
  dock 现在不可关闭、可移动/浮动；主工具栏 `right_panel_toggle_button` 可隐藏/显示右侧
  AI 面板并保留展开宽度；自定义 dock 标题栏的 `right_dock_float_button` 支持弹出独立窗口
  和归位到右侧栏。`tests/test_smoke.py` 已覆盖 `main_toolbar`、toggle 文案、dock 不可关闭、
  float/restore；本次补跑 `tests/test_smoke.py -q` 通过 11 项。
...
- `docs/tinybdmath_ai_math_latex_structure_scope.md`：AI/Math 第一阶段 LaTeX 数学排版结构范围。
...
- `src/ui/main_window.py`：主窗口、PDF 交互、右侧 AI 工具集 dock（全文问答/证据、
  隐藏/显示、浮动/归位）和后台公式/语义复核调度。
