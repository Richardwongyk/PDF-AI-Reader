...
0. 已完成 2026-06-05 UI 小步：左侧导航 dock 和右侧 `AI 工具集` dock 支持菜单栏同层
   小型线头按钮隐藏/显示，左右 dock 均支持标题栏弹出/归位且不可关闭；全局 tooltip
   为白字深底；默认启动走云端生成路由，不再弹 Ollama；中央 PDF 阅读区支持按需横向滚动、
   侧栏折叠后横向居中和 bbox 横向定位。翻译裂缝正文按原段落 bbox 设置左右 padding，
   split page 缩放/离屏恢复会按当前页面宽度重建，不再停在旧缩放宽度。
   顶部工具栏空白区和恢复细条支持双击折叠/恢复；菜单栏文字双击折叠未可靠实现，暂不继续。
   文档标签页/标签关闭当前文档暂不实现，只保留为后续可选方向。
