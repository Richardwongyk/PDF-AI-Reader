...
- 真实数据集工程继续增强：新增确定性源码 math span 扫描器、页级分片/断点续跑构建器、分片合并器、gold/label tier 审计和一键流水线脚本。LaTeX source gold 只用于训练/验收；PDF 候选标签加入 TOC 页锚点窗口，优先同页/近页匹配，短公式也受页窗口约束，避免全书范围误配。当前分片版本为 `tinybdmath_pdf_source_page_anchor_v2`，旧分片不会被合并器误用。Attention 15 页和 Napkin 第 9-16 页 smoke 已通过；v2 全量构建在 `test_artifacts/tinybdmath_sharded_full_page_anchor_v2` 后台运行，测试产物不提交。
