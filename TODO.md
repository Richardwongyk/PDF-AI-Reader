...
- 第一阶段 PDF 解析器研发已开始：新增 `src/core/pdf_glyph_graph.py`，把 PyMuPDF born-digital facts 标准化为 `RawGlyphGraph`，包含 glyph/font/bbox/origin/cid、vector/image 节点、line/span 读序边、PDF health、稳定 input hash；`BornDigitalFormulaStructureExtractor` 的 r0 candidate evidence 现在携带局部 raw graph schema/hash/health/glyph/vector/image，供后续 r0.5 符号身份修复和 TinyBDMath 使用。新增 `tests/test_pdf_glyph_graph.py`，相关公式集合为 `180 passed`。
