...
class QAService:
    ...
    QA_SYSTEM_PROMPT: str = (
        ...
        "## 规则\n"
        "1. 优先依据以下带编号的文档证据回答问题，证据编号形如 [S1]、[S2]。\n"
        "2. 如果问题超出文档证据范围，可以使用你的通用学术背景知识补充，但必须明确标注“背景补充”。\n"
        "3. 不得把背景知识说成论文原文内容；论文中没有明确给出的内容要说明“本文未直接说明”。\n"
        "4. 每个基于文档的关键结论后引用对应证据编号，例如 [S1] 或 [S2][S4]；背景补充不要伪造引用。\n"
        "5. 回答中使用 LaTeX 格式展示数学公式：行内用 \\(...\\)，行间用 $$...$$。\n"
        "6. 若涉及推导，逐步展示中间步骤。\n"
        "7. 能引用具体章节、定理编号或页码时尽量引用。"
    )
    ...
    def _build_qa_messages(
        ...
        if context_parts:
            ...
            context_text = (
                "（当前没有可引用的文档片段。请说明未检索到足够的本文档证据；"
                "若问题适合用通用学术背景回答，可以在“背景补充”中回答。）"
            )
...
class MockLLMClient(BaseLLMClient):
    ...
    def _mock_qa(self, text: str) -> str:
        ...
        return (
            ...
            f"如果您需要更深入的解释，请尝试：\n"
            f"1. 在右侧 AI 工具集中提出更具体的问题\n"
            f"2. 配置云端API获取真实的AI回答\n\n"
            f"（注：当前为测试模式。）"
        )
