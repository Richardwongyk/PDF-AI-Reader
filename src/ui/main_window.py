...
class MainWindow(QMainWindow):
    ...
    def __init__(self, services: ServiceContainer) -> None:
        ...
        self._reader_tab_bar: QTabBar | None = None
        self._reader_tab_strip: QWidget | None = None
        self._new_reader_tab_button: QToolButton | None = None
        self._reader_tabs: list[_ReaderTabState] = []
        self._active_reader_tab_id: int | None = None
        self._next_reader_tab_id: int = 1
    ...
    def _create_menu_bar(self) -> None:
        ...
        new_tab_action = QAction("新建阅读标签(&T)", self)
        new_tab_action.setShortcut(QKeySequence("Ctrl+T"))
        new_tab_action.triggered.connect(self._on_new_reader_tab)
    ...
    def _create_central_widget(self) -> None:
        ...
        self._new_reader_tab_button = QToolButton()
        self._new_reader_tab_button.setObjectName("new_reader_tab_button")
        ...
        self._create_reader_tab(switch=True)

    def _create_reader_tab(
        self,
        *,
        title: str = "新标签",
        filepath: str = "",
        ...
        tab = _ReaderTabState(
            tab_id=self._next_reader_tab_id,
            title=title,
            filepath=filepath,
            doc_hash=doc_hash,
        ...
        if self._reader_tab_bar is not None:
            ...
            if switch:
                ...
                try:
                    self._reader_tab_bar.setCurrentIndex(index)
                finally:
                    self._suppress_reader_tab_change = False
        if self._active_reader_tab_id is None:
            self._active_reader_tab_id = tab.tab_id
        self._sync_reader_tab_bar()
        return tab

    def _reader_tab_by_id(self, tab_id: int | None) -> _ReaderTabState | None:
        if tab_id is None:
            return None
        for tab in self._reader_tabs:
            if tab.tab_id == tab_id:
                return tab
        return None

    def _active_reader_tab(self) -> _ReaderTabState | None:
        return self._reader_tab_by_id(self._active_reader_tab_id)

    def _reader_tab_index(self, tab_id: int | None) -> int:
        if self._reader_tab_bar is None or tab_id is None:
            return -1
        for index in range(self._reader_tab_bar.count()):
            if self._reader_tab_bar.tabData(index) == tab_id:
                return index
        return -1

    def _set_active_reader_tab(self, tab_id: int | None) -> None:
        self._active_reader_tab_id = tab_id
        if self._reader_tab_bar is None or tab_id is None:
            return
        index = self._reader_tab_index(tab_id)
        if index >= 0 and self._reader_tab_bar.currentIndex() != index:
            self._suppress_reader_tab_change = True
            try:
                self._reader_tab_bar.setCurrentIndex(index)
            finally:
    ...
    def _sync_reader_tabs_from_bar_order(self) -> None:
        if self._reader_tab_bar is None:
            return
        ordered: list[_ReaderTabState] = []
        seen: set[int] = set()
        for index in range(self._reader_tab_bar.count()):
            tab_id = self._reader_tab_bar.tabData(index)
            if not isinstance(tab_id, int):
                continue
            tab = self._reader_tab_by_id(tab_id)
            if tab is None:
                continue
            ordered.append(tab)
            seen.add(tab.tab_id)
        ordered.extend(tab for tab in self._reader_tabs if tab.tab_id not in seen)
        self._reader_tabs = ordered

    def _on_reader_tab_moved(self, _from: int, _to: int) -> None:
        self._sync_reader_tabs_from_bar_order()
        self._session_save_timer.start()

    def _clear_reader_tabs(self) -> None:
        self._reader_tabs.clear()
        self._active_reader_tab_id = None
        self._next_reader_tab_id = 1
        if self._reader_tab_bar is None:
            return
        self._suppress_reader_tab_change = True
        try:
            while self._reader_tab_bar.count():
                self._reader_tab_bar.removeTab(0)
        finally:
            self._suppress_reader_tab_change = False
    ...
    def _reader_tab_to_session_dict(self, tab: _ReaderTabState) -> dict[str, Any]:
        ...
        return {
            ...
            "doc_hash": tab.doc_hash,
            "position": dict(position),
        }

    def _save_active_reader_tab_state(self) -> None:
        ...
        if self._viewer_document_loaded:
            try:
                ...
                self.logger.debug("Failed to save reader tab position", exc_info=True)
            else:
                if position:
                    tab.position = position
        self._sync_reader_tab_bar()

    def _on_new_reader_tab(self) -> None:
        self._save_current_session_state()
        self._save_active_reader_tab_state()
        tab = self._create_reader_tab(switch=True)
        self._activate_reader_tab(tab.tab_id)

    def _on_reader_tab_changed(self, index: int) -> None:
        if self._suppress_reader_tab_change or index < 0 or self._reader_tab_bar is None:
            return
        tab_id = self._reader_tab_bar.tabData(index)
        if isinstance(tab_id, int):
            self._activate_reader_tab(tab_id)

    def _activate_reader_tab(self, tab_id: int) -> None:
        if tab_id == self._active_reader_tab_id:
            return
        current_tab = self._active_reader_tab()
        if current_tab is not None:
            self._save_active_reader_tab_state()
        target = self._reader_tab_by_id(tab_id)
        if target is None:
            return
        self._set_active_reader_tab(tab_id)
        self._reader_tab_activation_generation += 1
        generation = self._reader_tab_activation_generation
        if self._can_switch_reader_tab_without_reopen(target):
            self._restore_reader_tab_position(target)
            self._sync_reader_tab_bar()
            self._session_save_timer.start()
            return
        self._status_page_label.setText("正在切换标签...")
        QTimer.singleShot(
            0,
            lambda tab_id=tab_id, generation=generation: self._load_reader_tab_after_switch(
                tab_id,
                generation,
            ),
        )
    ...
    def _can_switch_reader_tab_without_reopen(self, target: _ReaderTabState) -> bool:
        return (
            bool(target.filepath)
            and self._viewer_document_loaded
            and bool(self._current_filepath)
            and self._same_session_path(target.filepath, self._current_filepath)
        )

    def _restore_reader_tab_position(self, target: _ReaderTabState) -> None:
        target.doc_hash = self._current_doc_hash
        if not target.position:
            return
        self._pending_session_restore = {
            "filepath": self._current_filepath or target.filepath,
            "doc_hash": self._current_doc_hash,
            "position": dict(target.position),
        }
        self._restore_pending_session_position()

    def _load_reader_tab_after_switch(self, tab_id: int, generation: int) -> None:
        if generation != self._reader_tab_activation_generation:
            return
        if tab_id != self._active_reader_tab_id:
            return
        target = self._reader_tab_by_id(tab_id)
        ...
        if target.filepath:
            self._open_pdf_file(
                ...
                tab_position=target.position,
            )
            return

        self._pending_session_restore = None
        if self._viewer_document_loaded or self._current_filepath:
            self._document_open_in_progress = True
            self._document_flow.close_document()
            self._document_open_in_progress = False
        else:
            self._on_document_closed()

    def _on_reader_tab_close_requested(self, index: int) -> None:
        if self._reader_tab_bar is None or index < 0:
            return
        tab_id = self._reader_tab_bar.tabData(index)
        if not isinstance(tab_id, int):
            return
        closing_active = tab_id == self._active_reader_tab_id
        if closing_active:
            self._save_current_session_state()
            self._save_active_reader_tab_state()
        self._reader_tabs = [tab for tab in self._reader_tabs if tab.tab_id != tab_id]
        self._suppress_reader_tab_change = True
        try:
            self._reader_tab_bar.removeTab(index)
        finally:
            self._suppress_reader_tab_change = False
        if self._reader_tab_bar.count() == 0:
            tab = self._create_reader_tab(switch=True)
            ...
            return
        if closing_active:
            new_index = min(index, self._reader_tab_bar.count() - 1)
            new_tab_id = self._reader_tab_bar.tabData(new_index)
            if isinstance(new_tab_id, int):
                self._activate_reader_tab(new_tab_id)
        self._sync_reader_tab_bar()
        self._session_save_timer.start()

    def _ensure_reader_tab_for_open(
        self,
        filepath: str,
        *,
        replace_current_tab: bool,
    ) -> _ReaderTabState:
        tab = self._active_reader_tab()
        if tab is None:
    ...
    def _apply_theme(self) -> None:
        ...
        self._apply_reader_tab_style(theme)
    ...
    def _apply_reader_tab_style(self, theme: str) -> None:
        if self._reader_tab_strip is None:
            return
        theme = self._normalize_theme_name(theme)
        if theme == "light_gray":
            bg, tab_bg, active_bg = "#dcdcdc", "#d4d4d4", "#f1f1f1"
            border, text, accent = "#bebebe", "#2a2a2a", "#5b9bd5"
        elif theme == "sepia":
            bg, tab_bg, active_bg = "#d4ae58", "#e2c374", "#f1d98f"
            border, text, accent = "#9e7831", "#12325c", "#12325c"
        elif theme == "black_white":
            bg, tab_bg, active_bg = "#1a1a1a", "#242424", "#303030"
            border, text, accent = "#3a3a3a", "#e8e8e8", "#8cb4ff"
        else:
            bg, tab_bg, active_bg = "#1e1e2e", "#2a2a3c", "#31314a"
            border, text, accent = "#3a3a5a", "#cdd6f4", "#3b82f6"
        self._reader_tab_strip.setStyleSheet(f"""
            QWidget#reader_tab_strip {{
                background: {bg};
                border-bottom: 1px solid {border};
            }}
            QTabBar#reader_tab_bar {{
                background: {bg};
                color: {text};
            }}
            QTabBar#reader_tab_bar::tab {{
                background: {tab_bg};
                color: {text};
            ...
            QToolButton#new_reader_tab_button:hover {{
                border-color: {accent};
                background: {active_bg};
            }}
        """)
    ...
    def _connect_signals(self) -> None:
        ...
        self._document_flow.document_closing.connect(self._on_document_closing)
    ...
    def _open_pdf_file(
        ...
        *,
        ...
        replace_current_tab: bool = False,
        save_current_tab: bool = True,
        tab_position: dict[str, Any] | None = None,
    ) -> None:
        ...
        if save_current_tab:
            self._save_current_session_state()
            self._save_active_reader_tab_state()
        tab = self._ensure_reader_tab_for_open(
            resolved,
    ...
    def restore_last_reading_session(self) -> bool:
        ...
        if isinstance(state.get("tabs"), list):
            return self._restore_reader_tabs_session(state)
    ...
    def _restore_reader_tabs_session(self, state: dict[str, Any]) -> bool:
        ...
        self._clear_reader_tabs()
        for tab_state in restored_tabs:
            self._create_reader_tab(
                title=tab_state["title"],
                filepath=tab_state["filepath"],
                doc_hash=tab_state["doc_hash"],
                position=tab_state["position"],
                switch=False,
            )

        try:
            active_index = int(state.get("active_tab_index", 0) or 0)
        except (TypeError, ValueError):
            active_index = 0
        active_index = max(0, min(active_index, len(self._reader_tabs) - 1))
        active_tab = self._reader_tabs[active_index]
        self._set_active_reader_tab(active_tab.tab_id)
        self._sync_reader_tab_bar()
        ...
        try:
            path = Path(active_tab.filepath).expanduser().resolve()
        except OSError:
            return True
        if not path.is_file():
            return True
        ...
        self._open_pdf_file(
            ...
            replace_current_tab=True,
            save_current_tab=False,
            tab_position=active_tab.position,
        )
        return True
    ...
    def _save_current_session_state(self) -> None:
        ...
        if (
            ...
            and self._current_doc_hash
        ):
            position: dict[str, Any] | None = None
            try:
                raw_position = self._pdf_viewer.current_reading_position()
            except Exception:
            ...
            if position:
                tab.position = position
        self._sync_reader_tabs_from_bar_order()
        if not self._reader_tabs:
            self._create_reader_tab(switch=True)
        active_tab = self._active_reader_tab()
        active_index = 0
        for index, candidate in enumerate(self._reader_tabs):
            if active_tab is not None and candidate.tab_id == active_tab.tab_id:
                active_index = index
                break
        tabs_payload = [self._reader_tab_to_session_dict(tab_state) for tab_state in self._reader_tabs]
        active_payload = tabs_payload[active_index] if tabs_payload else {}
        state = {
            "version": 2,
            "filepath": active_payload.get("filepath", ""),
            "doc_hash": active_payload.get("doc_hash", ""),
            "position": active_payload.get("position", {}),
    ...
    def _prepare_session_restore_for_path(self, filepath: str) -> None:
        ...
        if not self._same_session_path(filepath, str(state.get("filepath", "") or "")):
            ...
            candidate_tabs = tabs_state if isinstance(tabs_state, list) else []
            for tab_state in candidate_tabs:
                if not isinstance(tab_state, dict):
                    continue
                if not self._same_session_path(filepath, str(tab_state.get("filepath", "") or "")):
                    continue
                position = tab_state.get("position")
                self._pending_session_restore = {
                    "filepath": str(tab_state.get("filepath", "") or ""),
                    "doc_hash": str(tab_state.get("doc_hash", "") or ""),
                    "position": dict(position) if isinstance(position, dict) else {},
    ...
    def _on_document_closing(self) -> None:
        """DocumentFlow 通知文档即将关闭 → 先停止 PDF 渲染侧资源。"""
    ...
    def _on_document_closed(self) -> None:
        ...
        self._navigator.load_toc([])
        self._ai_doc_status.setText("未打开文档")
        self._ai_evidence_tree.clear()
        self._set_dock_answer_text("", finished=True)
        self._clear_dock_followups()
        self.setWindowTitle("PDF AI Reader")
        if not self._document_open_in_progress:
            tab = self._active_reader_tab()
            if tab is not None:
                tab.title = "新标签"
                tab.filepath = ""
    ...
    def closeEvent(self, event) -> None:
        ...
        # Stop document and background reading tasks through DocumentFlow.
        self._formula_index_flow.stop()
        self._formula_semantic_review_flow.stop()
        self._stop_formula_import_thread()
        self._document_flow.close_document()
        # Other singleton services are closed once by ServiceContainer in main.py.
