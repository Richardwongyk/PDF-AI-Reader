...
10. 发起一次基于全文检索的问答，检查检索和回答完成日志。
11. 再滚动一次，截图。
12. 收集日志尾部、错误/警告数量、关键行为计数和截图路径，写入 `test_artifacts/e2e/report.json`。
...
- 主窗口成功启动并保持响应。
- PDF 成功打开并记录 `文档加载完成`。
- 滚动、页码跳转、缩放、翻译折叠、知识库重建、问答动作完成并生成截图。
...
- 问答 UI 仍是裂缝内简单输入区，缺少全文证据面板、引用跳转、检索状态和建议问题闭环。
