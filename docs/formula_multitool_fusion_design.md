...
本设计解决一个核心问题：多轮公式解析不能只是把多个候选“各跑一遍”，而必须让
r0 PDF facts、r2 本机候选、r2a TinyBDMath 和 r3 语义复核在同一套证据、源码对照
和门禁体系下协作，逐轮提高 LaTeX 还原准确率。
...
3. r2/r2a 调用本机候选路径和 TinyBDMath 结构模型生成候选，并对同一公式区域做候选对齐、比较和复核。
...
- 不在 UI 热路径加载 OCR/MFR 或训练模型。

允许自写的只有工程层：

- 统一候选协议、任务调度、缓存 key、结果落库。
- PDF 事实抽取和跨候选 evidence 编排。
...
2026-05-28 追加边界：翻译公式保护同样不得走手写补丁路线。当前发现 `TranslationService`
共享 `TextPreprocessor` 可能导致并发流式翻译公式占位串线；正确修复是每个翻译请求独立 protection
session，并尽量原样恢复已明确标记的 LaTeX，而不是添加样本特化保护规则。
...
2. 每个候选来源写 `FormulaCandidate`，不覆盖。
3. 对每个候选计算：
   - 与 r0 文本证据的一致性。
   - 与其他候选来源的一致性。
...
- 如果一个候选来源比 r0 分数高但覆盖样本少，报告必须写明 `coverage_comparable=false`。
- r2/r2a 首轮冷启动慢，必须后续优化批处理和缓存，不得放入默认热路径。
...
- 只写候选 JSON。
- 不直接 accepted，不覆盖正文。
- 当 r2/r2a 候选冲突时，r3 必须解释选择依据或标记证据不足。
...
1. 默认 r0 报告生成低相似候选列表。
2. 将低相似、低置信、复杂二维结构、候选冲突排入 r2/r2a。
3. r2/r2a 对这些候选批量跑本机候选和 TinyBDMath 结构模型。
...
3. 2026-07-05 已拆除第三方公式工具 worker；PEK/MinerU/Paddle/Pix2Text 隔离环境
   不再由当前代码发现或调用。fusion 只消费本机持久化候选、TinyBDMath r2a
   候选和 r3 复核候选。
...
6. 下一步仍必须完成：
   - 完善 accepted/rejected/revision 产品级体验，尤其是批量审核和更清晰的路径证据。
   - 强化 r4/r5 GraphRAG 语义级抽取与路径证据。
   - 用 Attention/Napkin 大样本跑质量门禁和性能门禁。
   - 优化 r2/r2a 本机候选批处理，降低冷启动。
