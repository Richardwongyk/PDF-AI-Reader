...
- r0.5 不能被 TinyBDMath 或 OCR 替代。符号身份缺失时，必须先尽量利用 ToUnicode、font cmap、AGL、texglyphlist、TeX encoding、文档内传播和 outline/path 候选；这些证据不足时才降置信或转入视觉/人工兜底。

## 1.1 强制补丁顺序

后续实现必须按以下顺序尝试身份修复，并把每一步结果写入 summary/result JSON：

1. PDF 原生事实：已有 Unicode、ToUnicode CMap、ActualText/Tagged PDF/AltText。
2. Font table：embedded font cmap、OpenType cmap、OpenType MATH table 和 font program 诊断。
3. Glyph name mapping：AGL、AGLFN、zapfdingbats、texglyphlist、TeX encoding maps。
4. 文档内传播：同 normalized font、CID/glyph name、outline hash、几何尺寸和已知身份锚点传播；冲突时不修复。
5. Shape candidate：outline/path/template/轻量分类器只给候选，不直接 accepted。
6. 结构角色先验：只标注角色或风险，不凭上下文猜具体 Unicode。
7. 失败兜底：保留 unknown，写 warning，交给 r2b/r3/人工；不能静默编造符号。

任何阶段都不得使用 Attention/Napkin 专用词表、样本正则或手写一次性修复链。
