...
2026-05-31 补记上一轮未提交 bug fix：`src/ui/pdf_viewer.py` 的缩放裂缝页重渲染已按当前架构收窄。当前 viewer 不是完整 SplitLayer 架构，裂缝页状态分散在 `_VirtualPageLayout` split extra、`_page_segments`、`_splits`、`_split_pages`、QWidget layout 和 `_pending_split_rerenders` 中；因此判断优先级必须看 widget 是否仍挂在 layout，而不是只看页号是否在 `_split_pages`。新增 `_is_split_page_in_layout()` 后，`_set_zoom()` 只对当前 layout 中的裂缝页做即时段内 QLabel 缩放和异步清晰重渲染；离屏裂缝页只标记 stale pending，避免抢占可见页渲染队列。旧图即时缩放改用 `FastTransformation`，延迟恢复滚动位置改走 `_set_scroll_value_if_valid()`，防止 Qt 对象销毁后回调访问无效 scroll bar。新增 `tests/test_pdf_viewer_navigation.py::test_pdf_viewer_zoom_prioritizes_visible_split_pages` 验证可见裂缝页优先、离屏页保持 pending。注意：这不是 Napkin 大页面 tile-only 首帧黑底/空白 P0 的完整修复。

2026-05-31 右侧全文问答体验修复：本轮只修 QA/UI 的确定问题。`QAService` 仍优先使用检索证据，但检索不足时可结合全文上下文和模型背景知识补充，并要求区分文档证据与背景推断；云端 DeepSeek/LiteLLM 连接失败时会显示可读提示。右侧全文问答回答区已从 `QTextBrowser` 切到现有 `markdown_template.html` + KaTeX WebView 渲染链路，公式显示与裂缝问答/翻译一致；回答区按实际面板 palette 生成高对比配色，深背景白字、浅背景黑字。“正在检索全文知识库...”只作为临时占位，不进入 `_dock_answer_text`，首个回答 token 到达后会被真实内容替换。已跑 `tests/test_smoke.py::test_main_window_smoke`，并做过右侧回答 KaTeX/颜色 DOM smoke。尚未处理 KaTeX 不支持或模型输出错误导致的个别符号缺失。

2026-06-01 公式扫描/识别链路修复：本轮只修确定的数据断点。外部公式 worker 会保留 `pdf_path/page_num/bbox` 上下文，MinerU 等按 PDF 页面工作的后端不再在入口处丢失页面信息；`formula_index_jobs` 和 `formula_page_scan_jobs` 的主键已迁移为包含 `scan_round`，同一块/同一页的 r1 缓存识别和 r2 本地高精度精扫可以并存，不再互相覆盖；同轮次同内容任务保持幂等。fusion 派生 targeted r2 时只补扫缺少 local precise 的区域，已有 local precise 但质量不足的区域转入 r3/审核，不重复跑同一精扫。已跑公式链路测试 69 passed。注意：这不是公式还原质量达标，只是让扫描/识别任务能正确排队、透传上下文、按轮次落库。

2026-06-01 TinyBDMath 性能/评估/清理补充：已提交 `5b5b38e Optimize TinyBDMath relation scoring pipeline`，范围是 relation scorer PyTorch batch/vectorized 打分、compact score、direct structural decode、no-score-jsonl、streaming structural/decoded eval 和对应测试；没有在 decoder 里写样本硬编码。全量 direct eval 未继续训练，输出保留在 `test_artifacts/tinybdmath_direct_full_eval_20260601`，耗时约 192.59s，rows=29881，selected relations=202364，structural precision=0.971798、recall=0.188380、F1=0.315585，decoded exact_match_rate=0.523242、near_match_rate=0.659550、avg_similarity=0.839214。结论：性能路径已经更直接，但质量远未达标，下一步应做监督/模型/decoder/verifier，不得硬编码样本规则。目录清理已删除旧 fast compact 输出、probe、旧审计/压测日志、知识库缓存、Python cache 和插桩临时 work；保留 graph rows、instrumented JSONL、relation labels/mathml/edge model 和 direct full eval。`.pytest_cache` 权限异常且体积接近 0，未强行改 ACL。
...
当前停工红线：用户已明确要求“不要再工作了，除非你非常确定能改掉一个 bug”。因此新会话不要
继续大规模推进原计划，不要再启动高压测、算法优化、工具安装或重构。只有在已经读完本交接、
能把问题收敛到单一具体 bug、修复路径非常确定、且能用小范围回归测试证明时，才允许动代码；
否则只汇报当前状态并等待用户明确新指令。
