...
2026-06-01 公式扫描/识别链路补记：r1/r2 等任务状态已按轮次隔离，外部工具 worker 会保留 PDF 页面和 bbox 上下文，targeted r2 只补扫缺少本地高精度候选的区域。这修复的是图片/扫描公式与显式精扫的排队、透传和落库基础，不改变 born-digital 默认非 OCR 边界，也不把低置信候选写入正文或知识库 accepted。

2026-06-01 TinyBDMath 性能/评估补记：relation scorer 已改为 PyTorch batch/vectorized fast path，并支持 compact score、direct structural decode、no-score-jsonl 和 streaming eval；提交为 `5b5b38e Optimize TinyBDMath relation scoring pipeline`。全量 direct eval 不再训练，基于现有 graph rows、relation labels 和 edge model 评估 29881 行，约 192.59s 完成，structural micro F1=0.315585，decoded exact_match_rate=0.523242、near_match_rate=0.659550。该结果证明性能路径可用，但公式质量远未达标。下一步应补强 relation hard labels、模型结构、decoder/verifier 和质量门禁，不能写样本硬编码规则。
...
重构方案入口：`docs/pdf_viewer_rendering_refactor_plan.md`。该方案明确翻译框“裂开插入”是核心交互合同，
后续 viewer 重构只能把 split 状态提升为独立层，不能改成侧边栏或牺牲交互形态来换性能。

2026-05-28 最新停工红线：用户要求“不要再工作了，除非你非常确定能改掉一个 bug”。后续默认应停止
主动推进，不再做大规模重构、压测、算法探索或工具安装。只有当下一步能明确限定为单一具体 bug、
...
- TinyBDMath 小模型研发第一阶段已从文档推进到可运行主干：早期 `realdata` pipeline 已废弃并删除，当前数据入口是 `tools/tinybdmath_sharded_dataset.py`、`tools/tinybdmath_shard_consolidate.py`、`tools/tinybdmath_instrumented_latex_dataset.py` 和 `tools/tinybdmath_instrumented_graph_dataset.py`；当前训练/评估入口是 `tools/run_tinybdmath_relation_pipeline.ps1`、`tools/tinybdmath_train_edge_baseline.py` 和 `tools/tinybdmath_train_edge_torch.py`。`src/core/tinybdmath_baseline.py` 与 `src/core/tinybdmath_torch_backend.py` 保留为旧质量 scorer/兼容测试资产，但新工作优先走 graph relation/structural pipeline。`src/app/tinybdmath_candidate_service.py` 已作为 r2a 非视觉结构候选服务接入 `tools/formula_multiround_pipeline.py --run-tinybdmath`，所有输出仍是 candidate-only，不污染正文/RAG。
...
- 最新真实数据 smoke：Attention 全量 15 页页锚点构建通过，11 个 PDF 候选全部带 `same_page_window`；Napkin 第 9-16 页分片 smoke 通过，2 个 PDF 候选均带 `same_page_window`。旧全量无页锚点合并曾得到 Attention+Napkin source formulas 30790、PDF candidates 2493、exact label 378、near label 118、weak label 770、unmatched label 1227；该结果只作为旧基线，不再作为 v2 训练集。当前已后台启动 v2 全量构建到 `test_artifacts/tinybdmath_sharded_full_page_anchor_v2`，测试产物不提交。
- 最新阶段合并结果：`test_artifacts/tinybdmath_sharded_full_page_anchor_v2` 已合并 Attention 15 页和 Napkin 1050 页分片，source formulas 30790、PDF candidates 2493、training rows 2493；在极严自动 gold 门禁下 verified gold rows 为 12，其余主要因 `strict_token_signature_mismatch`、`not_exact_similarity`、`not_same_page_window`、`source_not_unique_in_page_window` 等原因被挡下。`tinybdmath_review_queue.py --include-images` 已能导出真实 PDF crop、源码上下文、PDF evidence 和模型审核 prompt；空白决策模板经 `tinybdmath_apply_review.py` 应用时不会进入 gold，只保留自动 verified 行。
...
- 右侧全文问答回答区已接入现有 Markdown/KaTeX WebView 渲染链路，公式显示与裂缝问答/翻译一致；回答区会按实际面板 palette 设置深底白字或浅底黑字，避免不可读。
- “正在检索全文知识库...”只作为临时占位显示，不进入答案缓存；回答内容到达后会替换该提示。
- 云端模型连接失败会显示更可读的错误提示；检索结果不足时，QA 可以结合全文上下文和模型背景知识补充，但应区分文档证据和背景推断。
